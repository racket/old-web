...
#ELI: If 1, use only the guest account, and allow no modifications
$only_guest = 0;
#ELI: The URL where editing bugs is possible
$edit_url = "";
...
$VERSION = '4.00';
my $REVISION = (split(/ /, '$Revision: 1.1 $ '))[1];
my $GNATS_VERS = '0.0';
...
sub get_pr_url
{
  ...
  my $url = $q->url(-relative=>1) . "?cmd=$cmd&database=$global_prefs{'database'}";
  $url .= "&pr=$pr" if $pr;
  $url .= "&return_url=" . $q->escape($q->url(-path=>1,-query=>1,-relative=>0))
...
{
  my($viewaudit, $tmp) = @_;
  # PLT: we always want the audit trail
  $viewaudit = 1;
...
sub standard_query_page
{
  my $page = 'Standard Query';
  page_start_html($page);
  page_heading($page, 'Standard Query');
...
sub quick_query_page
{
  my $page = 'Quick Query';
  page_start_html($page);
  page_heading($page, 'Quick Query');
  print_stored_queries();
  print $q->start_form(), hidden_db(), hidden_debug(),
        $q->submit('cmd', 'submit query'),
        "<hr>", "<table>";

  print "<tr><td valign=top>$STATE_FIELD:</td>\n<td>";
  my $value_list=fieldinfo($STATE_FIELD, 'values');
  my @values=('any', @$value_list);
  print popup_or_scrolling_menu ($STATE_FIELD, \@values, $values[0]);
  print "<br />",
        $q->checkbox_group(-name=>'ignoreclosed',
                           -values=>['Ignore Closed'],
                           -defaults=>['Ignore Closed']);
  ...
  my @allcolumns;
  push (@allcolumns, "$STATE_FIELD");
  push (@allcolumns, "$SYNOPSIS_FIELD");
  push (@allcolumns, "$CATEGORY_FIELD");
  push (@allcolumns, "$LAST_MODIFIED_FIELD");
  push (@allcolumns, "$RELEASE_FIELD");
...
sub advanced_query_page
{
  ...
  page_start_html($page);
  page_heading($page, 'Advanced Query');
  print_stored_queries();
  print $q->start_form(),
	hidden_debug(),
        hidden_db();
...
sub display_query_results
{
  ...
  for ($fieldnames[0], @columns)
  {
    ...
    my ($query_string) = $q->url(-path=>1,-query=>1,-relative=>1) =~ m/^[^?]*\?(.*)$/;
    $query_string =~ s/(\w|-)+=;//g;
  ...
  my $url = $q->url(-path=>1,-query=>1,-relative=>1);
  ...
  print $q->hidden(-name=>'cmd', -value=>'store query', -override=>1),
        ...
        $q->hidden('return_url', $q->url(-path=>1,-query=>1,-relative=>0)),
        "\n</td>\n</tr>\n</table>",
        $q->end_form();
}
...
sub submit_stored_query
{
  ...
  else
  {
    ...
    my $query_url = $q->url(-relative=>0) . '?cmd=' . $q->escape('submit query')
          . ';' . $query_string;
    if ($debug)
    {
      print_header(),
...
sub main_page
{
  ...
  print one_line_form('Query Problem Reports:',
                      $q->submit('cmd', 'quick query'),
                      '&nbsp;', $q->submit('cmd', 'standard query'),
...
EOF
  ...
  $row .= qq(<span style="$bannerstyle $banner_fontsize2">User: $db_prefs{'user'}&nbsp;&nbsp;</span>)
                 if $db_prefs{'user'} && (! $only_guest);
  $row .= qq(<span style="$bannerstyle $banner_fontsize2">Access: $access_level</span>)
                 if $access_level;
  $row .= qq(\n</td>\n<td align="right">
           <a href="$url" style="$bannerstyle $banner_fontsize1"><b><i>$site_banner_text</i></b></a>
           </td>\n</tr>\n</table></td></tr>\n);

  $row2 = qq(<tr>\n<td colspan="2">);
  $row2 .= qq(<table border="0" width="100%" cellspacing="0" cellpadding="0"><tr><td>\n);
  ...
  $row2 .= qq(<td><a href="$createurl" style="$buttonstyle">Create</a></td>)
        if can_create();
  $row2 .= qq(<td><a href="$url&cmd=quick%20query" style="$buttonstyle">Quick Query</a></td>);
  $row2 .= qq(<td><a href="$url&cmd=standard%20query" style="$buttonstyle">Standard Query</a></td>);
  $row2 .= qq(<td><a href="$url&cmd=advanced%20query" style="$buttonstyle">Advanced Query</a></td>);
  $row2 .= qq(<td><a href="$url&cmd=logout" style="$buttonstyle">Log Out</a></td>)
        unless ($site_gnatsweb_server_auth) || $only_guest;
  $row2 .= qq(<td><a href="$url&cmd=help" style="$buttonstyle">Help</a></td>);
  $row2 .= qq(</tr></table>\n);
  $row2 .= qq(</td><td align="right">\n);
  if ($pr ne "" && $edit_url ne "") {
    $row2 .= qq(<table border="1" cellspacing="0" bgcolor="$site_button_background" cellpadding="3">\n);
    $row2 .= qq(<tr>\n);
    $row2 .= qq(<td><a href="$edit_url?cmd=edit&pr=$pr" style="$buttonstyle">Edit</a></td>);
    $row2 .= qq(</tr></table>\n);
  }
  $row2 .= qq(</td></tr></table>\n);
  $row2 .= qq(</td>\n</tr>\n);
...
sub main
{
        ...
        || !$db_prefs{'user'})
  {
    if ($only_guest) {
      ...
      print_header();
      login_page($q->url(-path=>1,-query=>1,-relative=>1));
      exit();
    }
  }
  ...
  elsif($cmd eq 'standard query')
  {
    print_header();
    initialize();
    standard_query_page();
  }
  elsif($cmd eq 'quick query')
  {
    print_header();
    initialize();
    quick_query_page();
